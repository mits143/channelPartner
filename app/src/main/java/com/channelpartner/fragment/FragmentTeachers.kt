...
import com.channelpartner.adapter.SubscriptionExpertAdapter
import com.channelpartner.adapter.SubscriptionTeacherAdapter
...
class FragmentTeachers : Fragment(), SubscripitionView.MainView {

    var presenter: SubscriptionPresenter? = null
    var adapter: SubscriptionTeacherAdapter? = null
    var dataList: ArrayList<Teacher>? = arrayListOf()
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        adapter = SubscriptionTeacherAdapter(
            context!!,
            dataList!!,
    ...
    override fun onSuccess(responseModel: Response<SubscrpitionResponse>) {
        if (responseModel.body() != null && responseModel.body()!!.classes.isNotEmpty()) {
            dataList!!.clear()
            dataList!!.addAll(responseModel.body()!!.teachers)
