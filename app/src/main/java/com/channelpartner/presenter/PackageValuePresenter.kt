...
class PackageValuePresenter : PackageValueView.MainPresenter {

    var context: Context? = null
    var mainView: PackageValueView.MainView? = null

    @NonNull
    var disposable: Disposable? = null

    constructor(context: Context?, mainView: PackageValueView.MainView?) {
    ...
    override fun loadDataClassbook(
        token: String
    ) {
        mainView!!.showProgressbar()
        ...
        if (NetWorkConection.isNEtworkConnected(context!!)) {
            ...
            disposable = ApiClient.instance
                ...
                .subscribe({ listResponse ->
                    ...
                    when (responseCode) {
                        200, 201, 202, 204 -> {
                            mainView!!.onSuccessClassbook(listResponse)
                        }
                        400, 401, 500 -> {
                            mainView!!.onError(responseCode)
                        }
